...
%
%   options.METIS_OPTION_CTYPE      Specifies the matching scheme to be used
%                                   during coarsening.
%           METIS_CTYPE_RM(0)       Random matching.
%           METIS_CTYPE_SHEM(1)     Sorted heavy-edge matching.
%
%   options.METIS_OPTION_IPTYPE     Determines the algorithm used during initial
%                                   partitioning.
%           METIS_IPTYPE_GROW(0)    Grows a bisection using a greedy strategy.
%           METIS_IPTYPE_RANDOM(1)  Computes a bisection at random followed by
%                                   a refinement.
%           METIS_IPTYPE_EDGE(2)    Derives a separator from an edge cut.
%           METIS_IPTYPE_NODE(3)    Grows a bisection using a greedy node-based
%                                   strategy.
%
%   options.METIS_OPTION_RTYPE      Determines the algorithm used for refinement.
...
%                                   is 1.
%
%   options.METIS_OPTION_NSEPS      Specifies the number of different separators
%                                   that it will compute at each level of nested
%                                   dissection. The final separator that is used
%                                   is the smallest one. Default is 1.
%
%   options.METIS_OPTION_NITER      Specifies the number of iterations for the
%                                   refinement algorithms at each stage of the
%                                   uncoarsening process. Default is 10.
%
%   options.METIS_OPTION_SEED       Specifies the seed for the random number
%                                   generator.
%
%   options.METIS_OPTION_MINCONN    Specifies that the partitioning routines
...
%           1                       Does not perform a 2-hop matching.
%
%   options.METIS_OPTION_CONFIG     Specifies that the partitioning routines
...
%
%   options.METIS_OPTION_COMPRESS   Specifies that the graph should be compressed
%                                   by combining together vertices that have
%                                   identical adjacency lists.
%           0                       Does not try to compress the graph.
%           1                       Tries to compress the graph.
%
%   options.METIS_OPTION_CCORDER    Specifies if the connected components of the
%                                   graph should first be identified and ordered
%                                   separately.
%           0                       Does not identify the connected components.
%           1                       Identifies the connected components.
%
%   options.METIS_OPTION_PFACTOR    Specifies the minimum degree of the vertices
%                                   that will be ordered last. If the specified
%                                   value is x>0, then any vertices with a degree
%                                   greater than 0.1*x*(average degree) are
%                                   removed from the graph, an ordering of the
%                                   rest of the vertices is computed, and an
%                                   overall ordering is computed by ordering the
%                                   removed vertices at the end of the overall
%                                   ordering. For example if x=40, and the
%                                   average degree is 5, then the algorithm will
%                                   remove all vertices with degree greater than
%                                   20. The vertices that are removed are ordered
%                                   last (i.e., they are automatically placed in
%                                   the top-level separator). Good values are
%                                   often in the range of 60 to 200 (i.e., 6 to
%                                   20 times more than the average). Default
%                                   value is 0, indicating that no vertices are
%                                   removed.
%                                   Used to control whether or not the ordering
%                                   algorithm should remove any vertices with
%                                   high degree (i.e., dense colmuns). This is
%                                   particularly helpfull for certain classes of
%                                   LP matrices, in which there a few  vertices
%                                   that are connected to many other vertices. By
%                                   removing these vertices prior to ordering,
%                                   the quality and the amount of time required
%                                   to do the ordering improves.
%
%   options.METIS_OPTION_UFACTOR    Specifies the maximum allowed load imbalance
...
%
%   options.METIS_OPTION_DBGLVL     Specifies the amount of progress/debugging
...
%       Matrices, Version 5.1.0, George Karypis, University of Minnesota,
%       March 30, 2013.
%
