...
%
%   ncon        The number of balancing constrints. It should be at least 1.
%
...
%               size 2m, where m is the number of undirected edges in the graph.
%               adjncy stores the edge list of vertices.
%
%               For detailed description of xadj, adjncy, please read Section 5.5
%               in [1].
%
%   eptr        The adjacency structure of the mesh. eptr is similar to xadj.
%
%   eind        The adjacency structure of the mesh. eind is similar to adjncy.
%
%   vwgt        The weights of the vertices. Empty ([]) is allowed.
%
%   vsize       The size of the vertices for computing the total communication
%               volume. Empty ([]) is allowed.
%
...
%               Given two elements e1 and e2, containing n1 and n2 nodes,
%               respectively, then an edge will connect the vertices in the dual
%               graph corresponding to e1 and e2 if the number of common nodes
%               between them is greater than or equal to
%               min(ncommon, n1-1, n2-1).
%               The default value is 1, indicating that two elements will be
%               connected via an edge as long as they share one node. However,
%               this will tend to create too many edges (increasing the memory
%               and time requirements of the partitioning). The user should
%               select higher values that are better suited for the element type
%               of the mesh that wants to partition. For example, for
%               tetrahedron meshes, ncommon should be 3, which creates an edge
%               between two tets when they share a triangular
%               face (i.e., 3 nodes).
%
%   nparts      The number of parts to partition the graph.
%
%   tpwgts      This is an array of size nparts*ncon that specifies the desired
...
%               Empty ([]) is allowed to indicate that the graph should be
%               equally divided among the partitions.
%
%   ubvec       This is an array of size ncon that specifies the allowed load
...
%               1.01 (for ncon>1).
%
%   options     This is the array of options. For detailed information, readers
%               are referred to METIS_OPTIONS.
%
%   objval      Upon successful completion, this variable stores the edge-cut
%               or the total communication valume of the partitioning solution.
%               The value returned depends on the partitioning's objective
%               function.
%
%   part        This is a vector of size nvtxs that upon successful completion
...
%
%   perm        This is a vector of size nvtxs. Upon successful completion, it
%               stores the fill-reducing permutation.
%
%   iperm       This is a vector of size nvtxs. Upon successful completion, it
%               stores the fill-reducing inverse-permutation.
%
